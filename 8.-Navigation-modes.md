...
The neutral position of the throttle stick (hold current height ) is defined by “set nav_use_midthr_for_althold”:   
- “set nav_use_midthr_for_althold=ON”: use mid stick as neutral. 
- “set nav_use_midthr_for_althold =OFF”: use current stick position (i.e. when activating ALTHOLD) as neutral. [Yet, if "nav_use_midthr_for_althold=OFF”, and you enable ALTHOLD with throttle stick too low (like on the ground) iNAV will take “thr_mid” as a save default for neutral. “thr_mid” is defined In the “Receiver” tab and should be set to hover throttle.]   
...
Example:
Let's assume "nav_mc_hover_thr” is already set correctly to your copter's hover throttle and “set nav_use_midthr_for_althold =OFF”. Let's say you have your throttle stick at 30%, and you enter ALTHOLD, your copter will maintain hover at this 30%. If increase throttle up to 40% it will start to climb. (Even if your copter needs 60% throttle to actually climb up in normal flight without ALTHOLD.)

    
"set alt_hold_deadband = 50": You have to change throttle command (e.g. move throttle stick) by at least this amount to make the copter climb or decent and define a new neutal position for ALTHOLD.  
...
Make sure baro is well isolated. You may also want to reduce baro weight set iNAV_w_z_baro_p = 0.5  
Set nav_alt_p = 0 and try flying. This way the controller will attempt to keep zero climb rate without any reference to altitude. The quad should slowly drift either up or down. If it would be jumping up and down - nav_vel_* gains are too high.  
...
Now raise nav_vel_p to the edge of oscillations. Now AH should be almost perfect.  
...
If quad is buzzing while ALTHOLD is activated try lowering "VEL P" a bit.  

what is the trick with VEL -"I"?   
"This can be zero if hover throttle is precisely 1500us. Too much VEL I will lead to vertical oscillations, too low VEL I will cause drops or jumps when ALTHOLD is enabled, very low VEL I can result in total inability to maintain altitude"
VEL I is used to compensate for hover throttle being set to a slightly incorrect value. You can't set hover throttle to an exact value, there is always influence from thermals, battery charge level etc. VEL I is used to compensate for that. Setting it too high will generate Yo-Yo effect on its own.
  
To deal with oscillations you can try lowering your "AlT P", "VEL P", "nav_max_climb_rate", and "nav_manual_climb_rate".    
...
//ToDo: Explain the effect of “nav_mc_min_fly_thr”. "iNAV_w_z_baro_p = 0.5" # "weight" of baro reading
