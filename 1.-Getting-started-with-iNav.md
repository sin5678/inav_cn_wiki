...
* iNav has removed all other telemetry options when using Naze32 targets but LTM  and FrSky in official builds, if you need let's say SmartPort telemetry you need to compile your own. [See link](http://www.rcgroups.com/forums/showpost.php?p=34463103&postcount=4790). Eg. F3 board, which have more memory, have Smartport and HOTT included.

* iNav on F1 targets(example Naze32) only supports UBLOX GPS per default due to size limitation. If other are needed you need to modifiy and compile your own version.
