...
* iNav has removed all other telemetry options when using Naze32 targets but LTM in official builds, if you need let's say Frsky telemetry you need to compile your own. [See link](https://github.com/cleanflight/cleanflight/tree/master/docs/development). Eg. F3 board, which have more memory, have Frsky included.
...
* If your copter is toilet-bowling, which means, in the beginning it holds its position and then starts to make bigger and bigger circles, you probably have your magnetometer not calibrated correctly or it’s seeing the magnetic field of you power lines or the beeper. Try to place the the FC as far away as possible from the magnetic intererence causing parts e.g. mounting it on/under the top plate on small racers.
