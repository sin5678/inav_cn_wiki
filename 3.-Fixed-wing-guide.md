...
* Flash newest iNav. (Warning: iNav default mixer is Quadcopter, so it sends out signal that is for ESC. This can damage servo if servos are powered up (Example if you flash with an FTID adapter that powers up entire board instead of USB that does not power up the 5v line.) If flashing with FTID adapter disconnect your servos.
