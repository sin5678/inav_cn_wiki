This version has some significant differences from 1.4, please read the release notes carefully and upgrade INAV Configurator available at [Chrome Store](https://chrome.google.com/webstore/detail/inav-configurator/fmaidjmgkdkpafmbnmigkpdnpdhopgel).
...
INAV now features Parameter Group configuration storage. No more configuration resets when flashing a new version - only parts of the config that have changed will be reset to defaults. New `diff` command is designed to assist in finding how your configuration is different from default one.
...
Misconfigured or malfunctioning compass can confuse the firmware in such a way that it wont't realise where North direction is. This may cause the machine to fly in wrong direction when i.e. doing an RTH and fly away instead of returning to home.

Another case is flying in extremely strong wing which machine is unable to fight. In such case RTH will attempt to bring machine home but will never succeed and UAV will be blown away.
...
- PIXRACER board initial support. Experimental - many features are not yet functuonal
- OMNINUS F4 SD target
- F4BY target
...
- Time overflow at 1 hour, 11 minutes, 35 seconds - fixed only on F3 and F4 targets.
